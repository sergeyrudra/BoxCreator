...
public class Box13 extends AbstractBox {
	@Override
	...
	public void create(DbWorker dbWorker) {
		...
		d = new Detail(CommonStatic.Details.BACK_MIDDLE, z-BoxSettings.getBoxSettings().getSettings(SettingsType.CLOSET_DEEP), dbWorker.y-BoxSettings.getBoxSettings().getSettings(SettingsType.SOCLE_Y)- (BoxSettings.getBoxSettings().getSettings(SettingsType.DSP_THICK) * 2), 0, 1, 1,dbWorker.quantity, BoxSettings.getBoxSettings()
				.getSettings(SettingsType.EDGE_THICK));// стенка
		details.add(d);
