...
public class Box1 extends AbstractBox {
	public void create(DbWorker dbWorker) {
		...
		d = new Detail(CommonStatic.Details.REAR, "dvp", dbWorker.x
				...
				- BoxSettings.getBoxSettings().getSettings(
						SettingsType.REAR_MARG), 0, 0, 1, dbWorker.quantity);// задняя
		details.add(d);
		d = new Detail(CommonStatic.Details.FACADE, dbWorker.x
				- BoxSettings.getBoxSettings().getSettings(
						SettingsType.FACADE_CLEARANCE), dbWorker.y
				- BoxSettings.getBoxSettings().getSettings(
